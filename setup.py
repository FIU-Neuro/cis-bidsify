...
def main():
    ...
    ver_file = op.join('bidsify', 'info.py')
    ...
    version = None
    cmdclass = {}
    if op.isfile(op.join(root_dir, 'bidsify', 'VERSION')):
        with open(op.join(root_dir, 'bidsify', 'VERSION')) as vfile:
            version = vfile.readline().strip()
        pkg_data['bidsify'].insert(0, 'VERSION')
